      ...
      {rightPanelOpen && (
        <>
          ...
          <motion.aside
            ...
            animate={{ width: typeof window !== 'undefined' && window.innerWidth < 640 ? '100vw' : 360, opacity: 1 }}
            exit={{ width: 0, opacity: 0 }}
            transition={{ duration: 0.25, ease: 'easeInOut' }}
            className="absolute lg:relative right-0 top-0 bottom-0 z-50 h-full bg-background lg:bg-muted border-l border-border/50 flex flex-col overflow-hidden shrink-0"
          >
            {/* Tab Bar */}
              ...
              {tabs.map((tab) => {
                ...
                return (
                  <button
                    ...
                    className={`flex-1 flex items-center justify-center gap-2 px-4 py-3 transition-colors relative ${isActive ? 'text-foreground' : 'text-muted-foreground hover:text-foreground'
                      }`}
                  >
